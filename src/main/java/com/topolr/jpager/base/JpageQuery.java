...
import com.topolr.mvc.data.jdbc.pool.DBPool;
...
public class JpageQuery {
    ...
    public HashMap<String, Object> queryTree(String sql, Object[] parameters, HashMap<String, Object> result) throws Exception {
        List<HashMap<String, Object>> list = this.queryList(sql, parameters);
    ...
    public int getRowCount(String sql, Object[] parlist) throws Exception {
        ...
        if (count == count2) {
            ...
            Logger.getLogger(JpageQuery.class).info("the arguments size can not match the sql.");
            return -1;
        }
    }

    public final <T> List<T> queryListObject(T obj, String sql, Object[] parlist) throws Exception {
        int count = this.times(sql);
        ...
        if (count == count2) {
            ...
            try {
                ...
                while (rs.next()) {
                    HashMap<String, Object> rsTree = new HashMap<String, Object>(numberOfColumns);
                    for (int r = 1; r < numberOfColumns + 1; r++) {
                        rsTree.put(rsmd.getColumnLabel(r), rs.getObject(r));
                    }
                    T c = (T) obj.getClass().newInstance();
                    ObjectSnooper.snoop(c).set(rsTree);
                    list.add(c);
                }
                return list;
            } finally {
    ...
    public final <T> T queryObject(T obj, String sql, Object[] parlist) throws Exception {
