...
public class SimpleRequest {
	...
	public SimpleRequest addText(String key, String text,
			ContentType contentType) {
		this.texts.put(key, new xtext(text, contentType));
		return this;
	}

	public SimpleRequest addFile(String key, String path,
			ContentType contentType) {
	...
	public HttpEntity geEntity() throws Exception {
		...
		if (text != null) {
			...
			for (Entry<String, xfile> x : this.files.entrySet()) {
				...
				reqEntity.addPart(x.getKey(),
						new FileBody(file.file, file.type));
			}
			for (Entry<String, String> xx : this.parameters.entrySet()) {
				reqEntity.addPart(xx.getKey(), new StringBody(xx.getValue(),
						ContentType.DEFAULT_TEXT));
			}
			for (Entry<String, xtext> xxx : this.texts.entrySet()) {
				reqEntity.addPart(xxx.getKey(), new StringBody(
						xxx.getValue().text, xxx.getValue().type));
			}
	...
	public static SimpleRequest getRequest() {
		...
		request.header.put("accept-language",
				"zh-CN,zh;q=0.8,en;q=0.6,ja;q=0.4,fr;q=0.2");
		request.header
				.put("user-agent",
		...
		request.header.put("content-type",
	...
	private CloseableHttpClient getSSLClient() throws Exception {
		...
		SSLContext ctx = new SSLContextBuilder().loadTrustMaterial(null,
				new TrustStrategy() {
					@Override
		...
		SSLConnectionSocketFactory socketFactory = new SSLConnectionSocketFactory(
				ctx, SSLConnectionSocketFactory.STRICT_HOSTNAME_VERIFIER);
	...
	private void setResponse(HttpClientContext context,
			CloseableHttpResponse response, SimpleResponse jresponsex)
			throws Exception {
