...
public class ServiceBootListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
        File file = (new File(ServiceBootListener.class.getClassLoader()
                .getResource("bootservice.json").getFile()));
        try {
            ...
            json.get("services").each(new JsonEachArray() {
                ...
                public boolean each(int index, Jsonx json) {
                    try {
                        ...
                        map.put(name, new BootService(name, className,
                                (IBootService) Class.forName(className)
                                .newInstance(), json.get("options")));
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                    return false;
            ...
            Logger.getLogger(ServiceBootListener.class.getName()).log(Priority.ERROR, ex);
        }
        this.boot();
    }
    ...
    private void boot() {
        for (Entry<String, BootService> x : map.entrySet()) {
            ...
            Logger.getLogger(ServiceBootListener.class.getName()).info("[service boot: " + x.getKey() + " startup]");
        }
    }
    ...
    private void stop() {
        for (Entry<String, BootService> x : map.entrySet()) {
            ...
            Logger.getLogger(ServiceBootListener.class.getName()).info("[service boot: " + x.getKey() + " shutdown]");
        }
    }
    ...
    public class BootService {
        ...
        public BootService(String name, String className, IBootService service,
                Jsonx options) {
            this.name = name;
